...
int main(int argc, char* argv[])
{
    ...
    std::string file, type, zmq_address, pub_address;
    uint16_t port;
    int hwm;
    ...
    desc.add_options()
        ...
        ("pub-address", po::value<std::string>(&pub_address)->default_value("localhost"), "DIFI ZMQ PUB address")
    ...
    // PUB
    void *difi_pub = zmq_socket(context, ZMQ_PUB);
    connect_string = "tcp://*" + pub_address + ":" + std::to_string(port+1);
    rc = zmq_bind(difi_pub, "tcp://*:50101");
    assert (rc == 0);
    ...
    while (not stop_signal_called
        ...
        if (start_rx and difi_packet.data) {
            ...
            for (uint32_t i = 0; i < difi_packet.num_rx_samps; i++) {
                ...
                double real = re;
                double imag = img;
                double div = 32768.0*32768.0*32768.0;
                ...
                img = imag/div;
                // restore in buffer
                memcpy((char*)&buffer[difi_packet.offset+i], &re, 2);
                memcpy((char*)&buffer[difi_packet.offset+i]+2, &img, 2);
            }
        ...
        // Forward
        zmq_send(difi_pub, (const char*)&buffer, len, 0);
