...
int main(int argc, char* argv[])
{
    ...
    std::string file, type, zmq_address, udp_forward;
    ...
    desc.add_options()
        ...
        ("udp-address", po::value<std::string>(&udp_forward)->default_value("127.0.0.1"), "UDP forward address")
        ("udp-port", po::value<uint16_t>(&udp_port)->default_value(8877), "UDP port")
    ...
    if (vm.count("help")) {
        std::cout << boost::format("VRT samples to UDP. %s") % desc << std::endl;
        std::cout << std::endl
    ...
    while (not stop_signal_called
        ...
        if (start_rx and vrt_packet.data) {
            ...
            for (uint32_t i = 0; i < blocks; i++) {
                if (sendto(sockfd, (char*)&float_data[i*1000*2], 1000*2*sizeof(float), 0,
