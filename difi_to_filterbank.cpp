...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("num-bins", po::value<uint32_t>(&num_bins)->default_value(1024), "number of bins")
        ...
        if (start_rx and difi_packet.data) {
            ...
            if (int_second) {
              ...
              if (first_frame) {
                ...
                keyword = "fch1";
                double_value = (double)difi_context.rf_freq/1e6-(double)difi_context.sample_rate/2e6;
                len = strlen(keyword);
                fwrite( &len, sizeof(len), 1, write_ptr);
                fwrite( (char*)keyword, len, 1, write_ptr);
                ...
                keyword = "tstart";
                double_value = ((double)difi_packet.integer_seconds_timestamp+(double)difi_packet.fractional_seconds_timestamp/1e12)/86400.0 + 40587.0;
                len = strlen(keyword);
                fwrite( &len, sizeof(len), 1, write_ptr);
                fwrite( (char*)keyword, len, 1, write_ptr);
                fwrite( &double_value, sizeof(double_value), 1, write_ptr);
