...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()
        ("help", "help message")
        ("file", po::value<std::string>(&file)->default_value("fifo.cf32"), "name of the fifo to write samples to")
    ...
    if (vm.count("help")) {
        std::cout << boost::format("VRT samples to fifo buffer. %s") % desc << std::endl;
        std::cout << std::endl
