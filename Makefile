...
# VRT IQ tools
all: clients dt
clients: vrt_version vrt_fftmax vrt_to_sigmf sigmf_to_vrt play_vrt vrt_forwarder vrt_spectrum vrt_to_void control_vrt vrt_to_rtl_tcp vrt_fftmax_quad vrt_to_filterbank vrt_to_fifo vrt_pulsar vrt_to_udp vrt_metadata vrt_to_stdout vrt_tuner vrt_correlate vrt_merge vrt_channelizer vrt_quantize vrt_buffer vrt_to_vdif
sdr: usrp_to_vrt rfspace_to_vrt rtlsdr_to_vrt airspy_to_vrt iio_to_vrt hackrf_to_vrt
...
vrt_to_vdif: src/vrt_to_vdif.cpp
		${CXX} -O3 $(INCLUDES) $(LIBS) $(CFLAGS) $(GIT_DEFINES) -o vrt_to_vdif src/vrt_to_vdif.cpp \
		-lvrt -lzmq $(BOOSTLIBS)
